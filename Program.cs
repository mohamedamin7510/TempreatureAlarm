...
Alarm alarm = new Alarm() { alarmedvalue = 30};
sensor.SensorChanged +=display.DisplayFiring; 
sensor.SensorChanged += alarm.alarmfiring;
...
while (true)
{
    ...
    switch (result)
    {
        case "1":
            ...
            sensor.SetTeampreature(temp_value);
            break;

        case "2":
            Console.WriteLine("Please enter ur alarm value : ");
            var alarm_value = int.Parse(Console.ReadLine()!);
            var x = new Alarm() {alarmedvalue = 100 };
            sensor.SensorChanged -= alarm.alarmfiring;
                  sensor.SensorChanged += x.alarmfiring;
            sensor.SetTeampreature(alarm_value);
